...
const Music: React.FC<MusicProps> = ({ music_questions }) => {
    ...
    const onAnswerSelected = (answer: any, idx: any) => {
        ...
        if (answer === music_questions[activeQuestion].correctAnswer) {
    ...
    const nextQuestion = () => {
        ...
        if (activeQuestion !== music_questions.length - 1) {
    ...
    return (
        <div className=' w-[100%] m-auto p-[3rem]'>
            <div className=' bg-white/50 p-4 text-center'>
                <div className=' mt-6 font-bold text-[2rem] text-slate-800 sm:text-[3rem]'>{music_questions[activeQuestion].title} Quiz</div>
                <div className=' font-semibold text-[1.1rem] mb-8'><span>Question: </span>{activeQuestion + 1}/{music_questions.length}</div>
                {!showResult ? (
                    <div className=' flex flex-col gap-10'>
                        <h3 className=' font-semibold text-[1.5rem]'>{music_questions[activeQuestion].question}</h3>
                        <div className=' flex flex-col gap-5'>
                            {
                                music_questions[activeQuestion].answers.map((answer: any, index: any) => (
                            ...
                            {checked ? (
                                <button className=' p-2 bg-slate-800 text-white' onClick={nextQuestion}>
                                    {activeQuestion !== music_questions.length - 1 ? "Next" : "Finish"}
                                </button>
                            ) :
                                <button className=' disabled:bg-white border-4 p-2'>{activeQuestion !== music_questions.length - 1 ? "Next" : "Finish"}</button>
                            }
                        </div>
                    </div>
                ) : (
                    <div className=' flex flex-col'>
                        ...
                        <div>Total Questions: {music_questions.length}</div>
